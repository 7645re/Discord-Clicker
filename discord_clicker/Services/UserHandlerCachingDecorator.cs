...
public class UserHandlerCachingDecorator : IUserHandler
{
    ...
    {
        User? user = null;
        if (userId != null && !_cache.TryGetValue(userId, out user) || name != null && password != null && !_cache.TryGetValue(name, out user))
        {
        ...
        if (user != null)
        {
            ...
            userModel = await _userHandler.GetUser(userId: user.Id, name: name, password: password);
            _cache.Set(user.Id+".UserModel", userModel,
                new MemoryCacheEntryOptions().SetPriority(CacheItemPriority.NeverRemove));
        }
        return userModel;
    }
