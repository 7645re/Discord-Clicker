...
public class ItemHandlerCachingDecorator<T, VT> : IItemHandler<T, VT> where T : class, IItem<T, VT>, new()
{
    ...
    private readonly string _itemsTCacheKey = typeof(T).Name;
    ...
    public async Task<Dictionary<bool, string>> BuyItem(int userId, int itemId, decimal money, DbSet<T> itemsContext)
    {
        ...
        if (!_cache.TryGetValue(_itemsTCacheKey, out List<T> items)) {
            ...
            _logger.LogInformation($"{_itemsTCacheKey} was loaded from database");
            _cache.Set(_itemsTCacheKey, items, new MemoryCacheEntryOptions().SetPriority(CacheItemPriority.NeverRemove));
        }
